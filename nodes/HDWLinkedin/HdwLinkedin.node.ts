import {
	IDataObject, IExecuteFunctions, IHttpRequestMethods,
	IHttpRequestOptions,
	INodeExecutionData,
	INodeType,
...
export class HdwLinkedin implements INodeType {
    ...
    options: [
        ...
        },
        {
            ...
            action: 'Get LinkedIn company posts',
            routing: {
                request: {
                    method: 'POST',
        ...
        // Add the new operation here:
        {
            ...
            action: 'Get LinkedIn company employee statistics',
            routing: {
                request: {
                    method: 'POST',
...
},
			{
				...
				options: [
					...
					},
					{
						...
						action: 'Search for LinkedIn jobs',
						routing: {
							request: {
								method: 'POST',
					...
					},
					{
						...
						action: 'Search for LinkedIn companies',
						routing: {
							request: {
								method: 'POST',
					...
					},
					{
						...
						action: 'Search for LinkedIn industries',
						routing: {
							request: {
								method: 'POST',
					...
					},
					{
						...
						action: 'Search for LinkedIn locations',
						routing: {
							request: {
								method: 'POST',
				...
				options: [
					{
						...
						action: 'Get LinkedIn group information',
						routing: {
							request: {
								method: 'POST',
...
},
{
  ...
  options: [
    ...
    },
    {
      ...
      options: [
        ...
        { name: 'Military and Protective Services', value: 'Military and Protective Services' },
		...
		try {
			...
			if (resource === 'user') {
				if (operation === 'search') {
					...
					if (additionalFields.company_keywords) body.company_keywords = additionalFields.company_keywords;
					if (additionalFields.school_keywords) body.school_keywords = additionalFields.school_keywords;
					if (additionalFields.current_company) body.current_company = additionalFields.current_company;
  ...
  const multiOptionFields = [
    'company_sizes', 'company_types', 'languages',
    'functions', 'levels',
    'years_in_the_current_company', 'years_in_the_current_position',
  ];
  ...
  if (additionalFilters.past_company_keyword) {
    body.past_company_keyword = additionalFilters.past_company_keyword as string;
					...
					if (additionalFilters.experience_level) body.experience_level = additionalFilters.experience_level;
					...
					if (additionalFilters.company) {
						if (typeof additionalFilters.company === 'string' && additionalFilters.company.includes(',')) {
					...
					if (additionalFilters.location) body.location = additionalFilters.location;
					if (additionalFilters.industry) body.industry = additionalFilters.industry;
					if (additionalFilters.employee_count) body.employee_count = additionalFilters.employee_count;
